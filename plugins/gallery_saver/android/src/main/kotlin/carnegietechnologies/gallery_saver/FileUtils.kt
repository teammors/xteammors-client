...
internal object FileUtils {
    ...
    fun insertImage(
        ...
        try {
            ...
            if (source != null) {
                ...
                outputStream?.use { it.write(source) }
    ...
    private fun getRotatedBytesIfNecessary(source: ByteArray?, path: String): ByteArray? {
        ...
        if (rotationInDegrees == 0) return null
    ...
    private fun storeThumbnail(
        ...
        var outputStream: OutputStream? = null
        try {
            outputStream = if (thumbUri != null) contentResolver.openOutputStream(thumbUri) else null
        } catch (_: Exception) {}
    }
    ...
    private fun getBytesFromFile(file: File): ByteArray? {
        ...
        buf.use { it.read(bytes, 0, bytes.size) }
        return bytes
    }
    ...
    fun insertVideo(
        ...
        if (toDcim) directory = Environment.DIRECTORY_DCIM
    ...
    private fun getAlbumFolderPath(
        ...
        albumFolderPath = if (TextUtils.isEmpty(folderName)) {
            var baseFolderName = if (mediaType == MediaType.image)
                Environment.DIRECTORY_PICTURES else Environment.DIRECTORY_MOVIES
            if (toDcim) baseFolderName = Environment.DIRECTORY_DCIM
            createDirIfNotExist(
    ...
    private fun createDirIfNotExist(dirPath: String): String? {
        ...
        if (!dir.exists()) {
            return if (dir.mkdirs()) dir.path else null
        } else {
            return dir.path
        }
    }
